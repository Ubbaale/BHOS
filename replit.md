...
-   **IT Services Dispatch & Ticketing**: A FieldNation-style system for healthcare IT dispatch. Users submit tickets with dispatch details (scheduled date/time, site address, on-site contact, equipment needed, special instructions, pay rate/type). Ticket categories include network, hardware, software, printer, EHR system, security, phone system, email, and backup. Dashboard shows ticket stats with filtering by status. FieldNation-grade features include: tech ETA tracking (en_route/arriving/on_site), check-in/check-out with time tracking, automatic payment calculation (hourly * hours or fixed rate, 10% platform fee), deliverables/proof of work uploads, two-way ratings (customer rates tech, tech rates customer), work order templates (save/load), talent pools for targeted routing, and routing modes (broadcast/talent_pool/direct_assign). Ticket owners see full job lifecycle: ETA status, check-in/out times, payment breakdown, proof of work, and rating forms. All routes under `/api/it/*`. Frontend at `/it-services`. DB tables: `it_companies`, `it_service_tickets`, `it_ticket_notes`, `it_talent_pools`, `it_talent_pool_members`, `it_work_order_templates`.
