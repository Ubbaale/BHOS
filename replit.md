...
-   **IT Services Dispatch & Ticketing**: A FieldNation-style system for small healthcare companies to submit IT service requests. Companies register a profile, then create tickets with dispatch details (scheduled date/time, site address, on-site contact, equipment needed, special instructions). Ticket categories include network, hardware, software, printer, EHR system, security, phone system, email, and backup. Dashboard shows ticket stats with filtering by status (open, in progress, resolved, closed). Notes/comments can be added to tickets. All routes under `/api/it/*` with company-ownership authorization. Frontend at `/it-services`. DB tables: `it_companies`, `it_service_tickets`, `it_ticket_notes`.
-   **IT Tech Onboarding & Dispatch**: Technicians apply at `/it-tech/apply` (public, no auth required) creating a user account with `it_tech` role and `it_tech_profiles` record. Applications go through admin approval (`/api/it/admin/techs/:id/approve|reject`). Approved techs access a dashboard at `/it-tech` showing available open tickets, active jobs, and completed work with stats. Ticket acceptance is atomic (race-safe). Tech apply uses DB transactions for consistency. Push notifications alert techs of new tickets. Routes: `/api/it/tech/*` for tech actions, `/api/it/admin/techs/*` for admin management.
