...
-   **Web Authentication**: A dedicated `/login` page for web users (admin, driver, patient) with role-based redirects after login (admins go to `/admin`, drivers to `/driver`, patients to `/`). Includes a multi-step forgot password flow at `/forgot-password` with 5-digit email codes via SendGrid. The Header shows "Sign In" for unauthenticated users and role-appropriate account links when logged in. Protected admin routes (`/admin`, `/admin/drivers`) redirect to `/login` when not authenticated.
